...
const ReferralModal: React.FC<ReferralModalProps> = ({ isOpen, onClose, doctorId, doctorProfile, patient, onSaved }) => {
  ...
  const handleApplyTemplate = (template: QuickTemplateData) => {
    if (template.diagnosis) {
      setReason(prev => prev.trim() ? prev : template.diagnosis!);
    }
  };

  const getReasonSaveData = (): QuickTemplateData | null => {
        ...
        <div>
          ...
          <div className="mt-2">
            <QuickTemplatesPanel
              ...
              type="complaint"
              label="Complaint shortcuts"
              isOpen={isOpen}
              onApply={handleApplyTemplate}
              getSaveData={getReasonSaveData}
            />
          </div>
        </div>
