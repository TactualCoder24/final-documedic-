...
const MentibotJournal: React.FC = () => {
    ...
    const prompts = [
        t('journal.prompt1', "What made you smile today?"),
        t('journal.prompt2', "Describe a challenge you faced and how you handled it."),
        t('journal.prompt3', "What are you grateful for in this moment?"),
        t('journal.prompt4', "How did you take care of yourself today?"),
        t('journal.prompt5', "What's one thing you'd like to improve tomorrow?")
    ];
    ...
    const handleSave = async () => {
        ...
        setTimeout(() => {
            setIsSaving(false);
            setEntry('');
            toast.success(t('journal.save_success', 'Journal entry saved successfully!'));
        }, 1500);
    };
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-3xl font-bold font-heading flex items-center gap-2">
                        ...
                        {t('journal.title', 'AI Journaling')}
                    </h1>
                    <p className="text-muted-foreground">{t('journal.subtitle', 'Release your thoughts and reflect on your inner world.')}</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-6">
                    <Card variant="premium" className="border-primary/20 overflow-hidden">
                        <CardHeader className="bg-primary/5 border-b border-primary/10">
                            <div className="flex justify-between items-center">
                                <CardTitle className="text-lg flex items-center gap-2">
                                    ...
                                    {t('journal.guided_reflection', 'Guided Reflection')}
                                </CardTitle>
                                <Button
                                    variant="ghost"
                                    size="sm"
                                ...
                                >
                                    ...
                                    {t('journal.new_prompt', 'New Prompt')}
                                </Button>
                            </div>
                        ...
                        <CardContent className="p-0">
                            <textarea
                                ...
                                placeholder={t('journal.placeholder', 'Start writing here...')}
                            ...
                            <div className="p-4 bg-muted/30 border-t border-border/50 flex justify-between items-center">
                                <span className="text-xs text-muted-foreground">{entry.trim().split(/\s+/).filter(x => x).length} {t('journal.words', 'words')}</span>
                                <div className="flex gap-2">
                                    ...
                                    <Button variant="gradient" onClick={handleSave} disabled={!entry.trim() || isSaving}>
                                        ...
                                        {t('journal.save_entry', 'Save Entry')}
                                    </Button>
                                </div>
                            </div>
                        </CardContent>
                    ...
                    <div className="flex items-center gap-2 p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20 text-amber-700 dark:text-amber-400 text-sm">
                        ...
                        {t('journal.ai_info', 'AI analysis is used to understand your journal sentiment privately to provide better wellness tips in your dashboard.')}
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    <div className="flex items-center justify-between">
                        <h2 className="text-xl font-bold font-heading flex items-center gap-2">
                            ...
                            {t('journal.recent_entries', 'Recent Entries')}
                        </h2>
                        <Button variant="ghost" size="sm" className="text-xs">{t('common.view_all', 'View All')}</Button>
                    </div>
                    ...
                    <Card variant="premium" className="bg-indigo-500/5 border-indigo-500/20">
                        <CardHeader>
                            <CardTitle className="text-lg">{t('journal.sentiment_report', 'Sentiment Report')}</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <div className="space-y-4">
                                <div className="flex justify-between text-xs font-bold">
                                    <span>{t('journal.positivity', 'POSITIVITY')}</span>
                                ...
                                <p className="text-[10px] text-muted-foreground">
                                    {t('journal.sentiment_insight', 'Your journals show a steady increase in positive sentiment over the last 3 entries. Great progress!')}
                                </p>
                            </div>
                        </CardContent>
                    </Card>
