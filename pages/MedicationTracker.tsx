  ...
  return (
    <>
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-6">
        <div>
          <h1 className="text-3xl font-bold font-heading">Medications</h1>
      ...
      <div className="space-y-6">
        <Card>
          ...
          <CardContent>
            {notificationPermission === 'granted' && <p className="text-sm font-medium text-green-600 dark:text-green-400">Notifications are enabled.</p>}
            {notificationPermission === 'default' && <Button onClick={handleEnableNotifications}>Enable Notifications</Button>}
            {notificationPermission === 'denied' && <p className="text-sm font-medium text-destructive">Notifications are blocked in your browser settings.</p>}
          </CardContent>
        </Card>
        ...
        <Card>
            ...
            <CardContent>
                <div role="progressbar" aria-valuenow={percentage} className="w-full bg-secondary rounded-full h-2.5">
        ...
        <Card>
          <CardHeader className="flex-row justify-between items-center">
            <CardTitle>Your Active Medications</CardTitle>
            <Button variant="outline" size="sm" onClick={() => setIsReportModalOpen(true)}>Report Not Taking</Button>
          </CardHeader>
          <CardContent>
                ...
                activeMedications.map(med => (
                  <div key={med.id} className={`flex items-center justify-between p-3 rounded-lg transition-colors ${med.takenToday ? 'bg-green-500/10' : 'bg-secondary/50'}`}>
                    ...
                    <div className="flex items-center gap-2">
                      <Button variant={med.takenToday ? 'outline' : 'default'} size="sm" onClick={() => toggleTaken(med.id)}>{med.takenToday ? 'Mark as Not Taken' : 'Mark as Taken'}</Button>
                    </div>
                  </div>
                ))
